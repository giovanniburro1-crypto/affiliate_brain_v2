"""
Упрощенный скрипт миграции для создания таблиц KnowledgeBaseV2.
"""
import sys
            ...
            if not check_table_exists(conn, "block_knowledge"):
                conn.execute(text("""
                    CREATE TABLE block_knowledge (
                        ...
                        -- Индексы
                        INDEX ix_block_knowledge_block_id (block_id)
                    )
                """))
            ...
            if not check_table_exists(conn, "block_vote_history"):
                conn.execute(text("""
                    CREATE TABLE block_vote_history (
                        ...
                        INDEX ix_block_vote_history_block_id (block_id)
                    )
                """))
...
def create_initial_learning_cycle():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(text("SELECT COUNT(*) FROM learning_cycles WHERE is_completed = 0"))
...
def main():
    ...
    print("=" * 60)
    print("МИГРАЦИЯ НА KNOWLEDGEBASE V2 (УПРОЩЕННАЯ)")
    print("=" * 60)
