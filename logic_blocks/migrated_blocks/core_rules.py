"""
core_rules - мигрированный блок из JSON.
"""
...
class CoreRulesBlock(KnowledgeBlock):
    """Блок знаний из core_rules.json"""
    
    def __init__(self):
        super().__init__(
            name="core_rules",
            description="Блок знаний из core_rules.json",
            weight=1.0,
            category="rules"
        )
  ...
  "killer_rules": {
    ...
    "roi_threshold": -20,
    "description": "Убиваем кампанию если потратили 2x payout без конверсий или ROI < -20%"
  },
  "scaler_rules": {
    "min_roi": 30,
    "min_conversions": 3,
    "stable_days": 3,
    "description": "Скейлим если ROI > 30%, conversions >= 3, и 3+ дня стабильно"
  },
  "optimizer_rules": {
    "epc_threshold": 0.15,
    "cpa_max": 5.0,
    "cr_min": 1.5,
    "description": "Оптимизируем если EPC низкий или CPA высокий"
  },
  "zacep_rules": {
    "min_conversions": 3,
    "min_stability_days": 2,
    "description": "Зацеп найден - изолируем в отдельный путь"
  }
}""")
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        return {
            ...
            "source_file": "core_rules.json"
        }


def register_block():
    return CoreRulesBlock()
